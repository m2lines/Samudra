...
#SBATCH --job-name=2026-07-16:samudra_llc:rb-Agulhas-BOUNDARY_CACHE+LLC_FULL_STORE---SPEED_TEST---
...
RESUME_CKPT_PATH="${RESUME_CKPT_PATH:-/orcd/data/abodner/002/cody/overflow/wandb_overflow/rb/2026-07-10:samudra_llc:rb-Agulhas-strides=1-pred_resid-eager-2-17608619/saved_nets/ckpt_25.pt}"
...
"${PROFILER_CMD[@]}" "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  -m ocean_emulators.train configs/samudra_llc/train_replay_003.yaml \
  --save_freq "${SAVE_FREQ}" \
  --epochs "${EPOCHS}" \
