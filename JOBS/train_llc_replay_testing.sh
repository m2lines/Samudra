...
#SBATCH --job-name=2026-07-05:samudra_llc:rb-quarter_face-train-test-reduced_ch_width-and-upscale_factor
...
DATA_LOCATION_OVERRIDE="${DATA_LOCATION_OVERRIDE:-/orcd/data/abodner/002/cody/LLC_patch/speed_test_QUARTER-of-FACE-LLC4320_face1_i0-2160_j0-2160_trainval_ready_20110913_20110927_t1.zarr}"
...
EMERGENCY_CHECKPOINT_INTERVAL_MINUTES="${EMERGENCY_CHECKPOINT_INTERVAL_MINUTES:-360}"
...
"${PROFILER_CMD[@]}" "${PYTHON_BIN}" -m torch.distributed.run \
  ...
  -m ocean_emulators.train configs/samudra_llc/train_replay_testing.yaml \
  --save_freq "${SAVE_FREQ}" \
  --epochs "${EPOCHS}" \
