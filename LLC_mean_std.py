...
def main():
    ...
    cluster = LocalCluster(
        n_workers=16,       # should be <= --cpus-per-task in job 
        threads_per_worker=1,
        memory_limit='60GB', # memory_limit * n_workers should be < --mem in job -------
    ...
    # set params -----------------------------------------------------------------------------------------------------------
    experiments = [2,4,12,24,48,96]# must be less than length, prescribes number of timepoints selected randomly per experiment
    ...
    for exp, num_time_samples in enumerate(experiments):
        ...
        mean_path = output_root / f'{num_time_samples}_LLC_means.zarr'
        std_path  = output_root / f'{num_time_samples}_LLC_stds.zarr'

        # only using 1 feature per type for testing ----------------------------------------------------------------------------
        ...
        for var in ubiquitous_features:
            ...
            std_da  = std_da.mean(dim='time_sampled')
        ...
        for var in surface_features:
            ...
            std_dict[var]  = std_da.mean(dim='time_sampled')
