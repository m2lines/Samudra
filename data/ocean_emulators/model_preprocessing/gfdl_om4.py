...
def convert_super_grid(ds_super_grid:xr.Dataset):
    ...
    b_indicies = dict(nyp=slice(0, None, 2), nxp=slice(0, None, 2))  # locations of 'bound variables required by xesmf
...
def om4_preprocessing(zarr_data_path, nc_grid_path, nc_mosaic_path):
    ...
    ds = ds.assign_coords(lon=ds_grid.geolon, lat=ds_grid.geolat, areacello=ds_grid.areacello)
    
    # drop (for now) all the coords on non-tracer position
    required_coords = ['lon', 'time', 'xh', 'lat', 'lev', 'yh', 'areacello', 'wetmask', 'dz']
