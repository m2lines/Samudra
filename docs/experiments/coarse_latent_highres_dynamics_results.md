...
| `2026-07-25-coarse-latent-s3-full-wx1-wz0.1-r6-resume-e7-normal-4x2` | S3 recovery | `1d940ce6` | Exact continuation from the r4 epoch-seven checkpoint on four nodes × two non-preemptible RTX6000s; eight workers, global batch 32, optimizer/scheduler/EMA, counters, W&B identity, and scientific configuration retained | Queued (`14769980`; validation `14769981`; audit `14769982`) | Routed explicitly to account `torch_pr_347_lzanna`, partition `rtx6000_lzanna`, without a preemption comment; test-only projected 16:03 EDT. Checkpoint SHA-256 is `2e973b22856eef621b6d1bc5f2e373a85832e059974ca4587739dc1e2527eb61` |
...
Epoch one completed in 33m45s and wrote a 976 MB recovery checkpoint. Its first
full-year validation is already better than the 768-update S2 result at every
lead:

| Checkpoint | Lead 1 | Lead 2 | Lead 4 | Persistence reduction |
|---|---:|---:|---:|---:|
...
persistence in S2. Epoch three improves all 12 route/lead cells again; the
...
Recovery chain `14760130`/`14760131`/`14760132` uses a new output directory but
...
the RTX6000 pool with `Requeue=1`. A scheduler-only comparison projected a
two-node × four-GPU placement at 13:36 EDT versus 15:17 for one node × eight;
both retain eight workers and global batch 32. The slower single-node request
was canceled before launch. The 2×4 recovery started at 11:03 EDT, initialized
all eight ranks across `gr102` and `gr103`, restored `Start Epoch: 5`, optimizer
LR 0.000530988, counters and W&B identity `c0smzwtd`, and passed first-batch
bring-up with finite losses. Because the interrupted process had emitted
...
configuration; only scheduler placement changes.
