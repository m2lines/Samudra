...
At native image size, each lossless PNG map panel is **720 × 240 pixels**,
with exactly **2 × 2 pixels per grid location** and no interpolation. Open the
image at full size to preserve this ratio; report previews may resize it. Cells
are square in grid-index space, with geographic coordinate labels. PDFs embed
the native grid without interpolation.
...
Each figure has a matching PDF in the [artifact index](artifacts/2026-09-23/README.md).
