...
**Evaluation span versus forecast horizon:** the table pools **96 separately
initialized monthly forecasts across January 2015–December 2022**. Each forecast
covers roughly one month; these are not errors after eight years of free-running
evolution. Surface errors average day-5/15/30 bin scores across that cohort; OHC
compares next-calendar-month means. Checkpoints were chosen on validation only.
Lower is better.

These are the eight-year **short-forecast aggregate** results. We have no continuous
eight-year rollout results for these observational pilot models. The
[complete table](artifacts/2026-09-23/heldout-table.md) and
[CSV](artifacts/2026-09-23/heldout-metrics.csv) contain the pooled results;
[summary.json](artifacts/2026-09-23/summary.json) includes paired year-by-year
score differences, and the [evidence archive](artifacts/2026-09-23/evidence.json.gz)
contains the full annual diagnostics.
`Combined score = 0.5 × mean(integrated error / fixed validation-climatology error) + 0.5 × mean(27 spatial spectral errors in dex)`.
Training RMSE does not select checkpoints. **Dex** means base-10 logarithmic units:
spectral error is the RMS difference between log₁₀ predicted and observed power,
averaged over the 27 variable/region/lead comparisons. A uniform factor-of-two
power mismatch gives 0.301 dex; a factor of ten gives 1 dex. It measures power
agreement, not phase or forecast timing.

Every method name below is defined in the [methods glossary](snapshot-2026-09-23-methods.md#result-table-method-names).
“Full model” in the persistence rows means the selected **full fine-tuned initializer**
with evolution bypassed; it is not another trained model.
...
## Model details and final rollout structure

Added from saved exports after the snapshot; scores and selected weights are unchanged.

D is a deterministic, non-diffusion system with two ConvNeXt-style U-Nets: a
**121.7M-parameter initializer** reconstructs two ocean states from 19 five-day
surface-history frames, and a **31.6M-parameter evolution network** advances those
states in five-day autoregressive steps. A shared **387-parameter** pointwise
8 → 32 → 3 adapter supplies ERA5 conditioning. Total size is **153.3M parameters**.
Full fine-tuning and observation-only scratch train the same architecture; the latter
starts from random weights. Adapter-only freezes the ocean networks and trains only
387 parameters. Architecture widths, interfaces, exact counts and optimizer settings
are given in the [paper-style methods description](snapshot-2026-09-23-methods.md#model-architecture-and-optimization).

The following maps show the **day-30 five-day mean**, after six autoregressive steps,
for the July 2022 case. January and July of the final held-out year were fixed as
seasonal examples before inspecting the maps. Rows compare observations, full
fine-tuning, scratch, adapter-only, source D, and matched initial-state persistence.
All rows use identical target-valid ocean support and fixed color limits. Gray is
land or missing observations; colorbar extensions indicate saturation.

| SST anomalies (°C) | ADT / SSH anomalies (m) |
|---|---|
| ![July day-30 SST anomalies](artifacts/2026-09-23/maps/day30-2022-07-anomalies-sst.png) | ![July day-30 ADT anomalies](artifacts/2026-09-23/maps/day30-2022-07-anomalies-adt.png) |

Anomalies subtract the **same training seasonal climatology** from observations and
every candidate. The July example shows the fine-tuned model retaining broad SST
anomaly structure, while scratch is smoother and the frozen-core controls exhibit
large seasonal departures. This is an illustrative case, not an additional skill
estimate; the full-cohort metrics above remain the basis for comparison.

Also available: July absolute [SST](artifacts/2026-09-23/maps/day30-2022-07-fields-sst.png)
and [ADT](artifacts/2026-09-23/maps/day30-2022-07-fields-adt.png); January
[SST anomalies](artifacts/2026-09-23/maps/day30-2022-01-anomalies-sst.png) and
[ADT anomalies](artifacts/2026-09-23/maps/day30-2022-01-anomalies-adt.png).
These are saved surface forecasts from separate monthly initializations, not the
end of a continuous eight-year rollout. Day 30 is the last common exported surface
lead; calendar-month interior aggregation can include a seventh five-day step.
