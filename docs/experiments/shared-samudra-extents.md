...
Updated 2026-09-16. **The campaign is complete; see the [final scientific report](velocity-transfer-final-2026-09-16/report.md).** This campaign implements the user's latest scope: **velocities only, local DUACS, OM4 auxiliary data, original convolutional Samudra**. LLC is unavailable and deferred. The eventual objective remains DUACS SSH evolution, but a positive velocity result does not establish SSH forecast skill or recover the missing SSH datum.
...
### D5 transition audit (2026-09-14 UTC)

The completed D5 screen used seed 15 and a 48 GPU-hour target. Its first 40% sampled global 1° and regional ¼° OM4 with equal probability per update, then switched entirely to DUACS. The actual run ended at 97% of the target, yielding approximately 4.8 hours of OM4 pretraining and 6.84 hours of DUACS fine-tuning on four GPUs. Global examples use two recursive training steps; regional examples use one. Explicit geometry is retained.

Only the backbone transfers between the two source adapters. At the transition, the DUACS input adapter remains randomly initialized and its residual output head remains zero-initialized; OM4 adapter weights are not copied. All DUACS-path parameters immediately train together, without a frozen-backbone adapter warmup. Backbone AdamW state and shared BatchNorm running statistics carry over. The continuous cosine learning-rate schedule is not restarted: the transition rate is approximately 2.07e-4, with no new warmup or smaller backbone rate. These details are verified in the pinned trainer and model source.

The live saved history confirms persistence-only DUACS predictions before the switch, followed by a best 10-day validation RMSE of 0.121392 m/s at update 179968, approximately 127 DUACS updates after pretraining. Subsequent sampled validation scores were approximately 0.14945 at 29.96 GPU-hours and 0.16595 at 42.99 GPU-hours, while training loss continued falling. This establishes failure of the tested transition recipe, not a general failure of OM4 pretraining. Untrained adapters, optimizer state, normalization changes and overfitting remain possible explanations. Adapter warmup followed by a smaller backbone learning rate and a fresh fine-tuning schedule is a proposed follow-up, not an executed experiment.

### Maintenance-aware completion (2026-09-15 UTC)

The user reported Torch maintenance starting September 16 at 07:00 ET (11:00 UTC). Both seed-15 confirmation runs are complete: D0 ended at 124.1602 training GPU-hours with best 10-day validation RMSE 0.10212355 m/s; D4 ended at 124.1603 with 0.10026465 m/s. D4 required a checkpoint resume after cluster cancellation. Seeds 16 and 17 remain in progress; cancelled allocations are recorded in the campaign manifest and resumed with the same model, optimizer, seed and training budget.

To reduce the evaluation work remaining after the last training run, the four unchanged full validation/test evaluations for completed seed-15 checkpoints were submitted early as jobs 17858613–17858616. These are part of the original twelve evaluations, not additional experiments. The remaining models retain their fixed training and validation-selection protocols. Evaluation controller 17858033 still waits for all six training runs, then reuses the recorded seed-15 evaluation IDs and submits the remaining evaluations. Queue delays and further preemptions may still push completion past maintenance; no result or completed budget is assumed from scheduling alone.

A subsequent preflight found immediate H100 capacity on the same lzanna account while these four RTX6000 evaluation requests remained pending. They were cancelled before starting and replaced by H100 jobs 17859574, 17859576, 17859577 and 17859578 (validation D0/D4, test D0/D4). All twelve full evaluations now request H100; confirmation training remains on RTX6000. Controller commit `112cf79c` adds a separate evaluation GPU constraint and preserves already submitted evaluation IDs; six controller tests passed. Replacement evaluation controller 17859579 waits for the unchanged training runs. Evaluator code and all checkpoint, cohort, metric and selection definitions remain pinned as before.

### Final completion (2026-09-16 UTC)

All six screens, six three-seed confirmation runs, and twelve full validation/test evaluations completed before maintenance. Each confirmation ended at approximately 124.16 measured training GPU-hours, the planned 97% stopping point of its 128-hour target. The final controller `17888644` marked the campaign complete after checking all evaluation manifests, recomputing comparisons, verifying the immutable code overlay, and collecting Slurm allocation accounting. Final artifacts and interpretation are in the [scientific report](velocity-transfer-final-2026-09-16/report.md).

Seed-15 evaluations used H100. Pending H100 jobs for the remaining seeds were cancelled before starting when predicted queue times passed the maintenance deadline; seed-16/17 evaluations completed on RTX6000 instead. Each D0/D4 seed pair uses the same evaluation hardware. Training code, selected checkpoints, precision, and evaluation cohorts were unchanged. A D4 seed-16 distributed validation failure was recovered from checkpoint; the same validation step subsequently passed with the expected persistence baseline. All final training/evaluation jobs exited successfully.

Pooled held-out 10-day improvement is +0.42% (paired seed/calendar-quarter 95% interval -1.98% to +3.30%); 30-day improvement is +7.45% (-0.07% to +15.34%). Five-day RMSE worsens by 2.38%. Thus the primary 3% criterion was not met and a reliable observational transfer benefit is not established. The final report includes the three-seed results, all mandatory baselines and regions, matched-exposure learning curves, checkpoint provenance, and 1,035.15 allocated GPU-hours by hardware family. No remote Rust-loader merge was performed.
