...
The pilots use 128 updates at most and target one GPU-hour each, including their small validation runs; D0 had a 45-minute wall limit. After D0 completed in 1m54s, the still-pending D3 job was shortened in place to a 15-minute wall limit to improve backfill eligibility, preserving its job ID, training workload, and downstream dependency. The controller requires completed pilots, finite validation results, selected checkpoints, and conservative memory margins before submitting the screen. Subsequent jobs use two sequential lanes of four GPUs, limiting simultaneous training to eight A100s. Stage transitions check Slurm allocation accounting against the remaining budget. Failures or incomplete artifacts prevent advancement.
