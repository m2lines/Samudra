...
# # Data Processing
#
# This notebook was run on a coiled notebook. To spin one up, please run: `coiled notebook start --container pangeo/pangeo-notebook --disk-size 256`

# In[1]:


get_ipython().system("pip install xpartition coiled h5netcdf")
...
data_output_directory = (
    "s3://fomo-data-eng/2025-06-13-data"  # "/pscratch/sd/s/suryad/Ocean_Emulator/data"
)
...
# More cloud-friendly way to load NetCDF data:
# see https://tutorial.xarray.dev/intermediate/remote_data/remote-data.html#reading-data-from-cloud-storage
fsspec_caching = {
...
"""Used to copy a working dataset to the OA buckets"""
# import zarr
...
try:
    ds_target_grid = xr.open_dataset(
        fs_osn.open(nc_target_grid_path, "rb", **fsspec_caching), engine="h5netcdf"
    )
    print("using h5netcdf")
except:
    ds_target_grid = xr.open_dataset(
...
cluster = coiled.Cluster(n_workers=16, scheduler_memory="128 GiB")
...
dest_name = f"{data_output_directory}/{dataset}_halfdeg.zarr"
dest_name
# {var: {"compressor": None} for var in ds.data_vars}
ds.to_zarr(
    f"{data_output_directory}/{dataset}_halfdeg.zarr",
