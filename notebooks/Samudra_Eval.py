...
pred_dict = {}
# pred_dict["pred_1"] = {
#     "name": "so-corrector-epoch117",
#     "run_name": "thermodynamic-scheduler-batchsize3-so-corrector-epoch117",
#     "path": "/pscratch/sd/s/suryad/Ocean_Emulator/.LOCAL/2025-02-15-eval-thermodynamic-scheduler-batchsize3-so-corrector-epoch117/120-infchmean0.035472/predictions_chunked.zarr",
#     "ls": ["thetao", "so", "uo", "vo", "tos", "zos"],
# }

pred_dict["pred_1"] = {
    "name": "cm4-om4-configs",
    "run_name": "cm4-om4-configs",
    "path": "/pscratch/sd/s/suryad/Ocean_Emulator/.LOCAL/2025-03-30-eval-cm4-with-om4-configs-inference-seed15/predictions.zarr",
    "ls": ["thetao", "so", "uo", "vo", "tos", "zos"],
}
...
output_path = (
    "/pscratch/sd/s/suryad/Ocean_Emulator/outputs/"
    + str(datetime.now())[:10]
    + "_"
...
old_ds_gt = xr.open_zarr(
    "/dvs_ro/cfs/cdirs/m4874/suryad/data/CM4_5daily_v0.4.0_preprocessed.zarr"
)
ds_prediction = xr.open_zarr(
    pred_dict[key1]["path"], chunks={"time": 10, "lat": 180, "lon": 360}
)
# ds_prediction = ds_prediction.chunk({"time": 10, "lat": 180, "lon": 360})
...
groundtruth_ls = var_ls + mask_vars
# This needs to be fixed upstream
# Hist 0
if ds_prediction.time.size == 2920:
    ds_groundtruth = ds_groundtruth[groundtruth_ls].sel(
        time=slice("0311-01-06", "0351-01-01")
    )
# Hist 1
elif ds_prediction.time.size == 2918:
    ds_groundtruth = ds_groundtruth[groundtruth_ls].sel(
        time=slice("0311-01-11", "0350-12-27")
    )
elif ds_prediction.time.size == 596:
    ds_groundtruth = (
        ds_groundtruth[groundtruth_ls]
        .sel(time=slice("0191-10-23", None))
        .isel(time=slice(None, 596))
    )
else:
    raise Exception(
...
if "model_path" in ds_prediction.attrs:
    pred_dict[key1]["model_path"] = ds_prediction.attrs["model_path"]

with open(os.path.join(output_path, "compare_info.txt"), "w") as f:
    f.write(json.dumps(pred_dict, sort_keys=True, indent=4))

pred_dict[key1]["ds_prediction"] = ds_prediction
...
# Combine 3D vars
def combine_all_variables_by_lev(ds, ls_3D=["thetao", "so", "uo", "vo"]):
    """
    Combine variables in the dataset along a new 'lev' dimension based on their suffix.

    Parameters:
    ds (xarray.Dataset): The input dataset containing variables with suffixes (e.g., thetao_0, so_1).
    prefixes (list): List of variable prefixes to combine.
...
pred_dict[key1]["ds_prediction"] = combine_all_variables_by_lev(
    pred_dict[key1]["ds_prediction"]
)
ds_groundtruth = combine_all_variables_by_lev(
...
# Constants
areacello = old_ds_gt.areacello.values
dz = old_ds_gt.dz.data
...
if "init_time" in data.coords:
    data = data.drop(["init_time", "valid_time"])


for var in data.data_vars:
    if "lev" in data[var].dims:
        data[var] = data[var].where(wetmask)
    else:
        data[var] = data[var].where(wetmask.isel(lev=0))

data["time"] = times
data["areacello"] = (["lat", "lon"], areacello)
data["dz"] = ("lev", dz)
...
for k in pred_dict.keys():
    ...
    pred_dict[k]["ds_prediction"] = pred_dict[k]["ds_prediction"].assign_coords(
        {co: data[co] for co in data.coords}
    )

    for var in pred_dict[k]["ds_prediction"].data_vars:
        if "lev" in pred_dict[k]["ds_prediction"][var].dims:
            pred_dict[k]["ds_prediction"][var] = pred_dict[k]["ds_prediction"][
                var
            ].where(wetmask)
        else:
            pred_dict[k]["ds_prediction"][var] = pred_dict[k]["ds_prediction"][
                var
            ].where(wetmask.isel(lev=0))
    ...
    pred_dict[k]["ds_prediction"]["time"] = times
    pred_dict[k]["ds_prediction"]["dz"] = ("lev", dz)
    pred_dict[k]["ds_prediction"]["areacello"] = (["lat", "lon"], areacello)
    pred_dict[k]["ds_prediction"] = pred_dict[k]["ds_prediction"].rename(
        {"lat": "y", "lon": "x"}
    )
...
for var in movie_var_list:
    ...
    data_gt = data_gt.where(data.mask.isel(time=0, lev=0)).compute()
    data_pred1 = data_pred1.where(data.mask.isel(time=0, lev=0)).compute()
    data_pred2 = data_pred2.where(data.mask.isel(time=0, lev=0)).compute()
