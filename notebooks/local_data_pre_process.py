...
vertical_dim = "lev"
vertical_idim = "ilev"
#rotation_angle = "angle" # no need as uo is already eastward and vo is laready northward (lat-lon)
...
#%%
# define local paths
data_dir = "/orcd/data/abodner/002/cody"  # directory containing local zarr/nc files

om_zarr_path = os.path.join(data_dir, "OM4.zarr") 
# ice_zarr_path = os.path.join(data_dir, "ice_5daily.zarr")
#nc_grid_path = os.path.join(data_dir, "ocean_static_no_mask_table.nc")  
#nc_mosaic_path = os.path.join(data_dir, "ocean_hgrid.nc")  
#nc_target_grid_path = os.path.join(data_dir, "gaussian_grid_360_by_720.nc")
#ocean_static_path = os.path.join(data_dir, "ocean_static.zarr")  
data_output_directory = os.path.join(data_dir, "processed_output")  
...
def main():
    ...
    ds = xr.open_zarr(om_zarr_path, chunks = chunking, consolidated=True)
    ds = ds.persist() # keep in memory for ops downstream
   ...
   #     print('no printable ds')
    ...
    coords = {
    "X": {"center": "x", "outer": "x_b"},
    "Y": {"center": "y", "outer": "y_b"},
    "Z": {"center": "lev", "outer": "ilev"},
    "T": {"center": "time"},
    } 
    ...
    ds = interpolate_to_cell_centers(ds, ds.thetao, grid)
    ds = ds.persist()
    find_tuples(ds)
    ...
    output_path = os.path.join(data_output_directory, f"{dataset}_processed_halfdeg.zarr")
    #print('try small save!')
    ds.isel(time=slice(0,10)).to_zarr("small_test___" + output_path, mode="w")
