...
with initialize_config_dir(
    ...
    args = compose(
        ...
        overrides=[
            "output_dir=./temp/{0}_conv_multiseed_g1g1_noise_fixed_100".format(
                str(datetime.now())[:10]
            ),
...
def load_long_data(e):
    ...
    if len(e.pred_paths) > 0:
        for model_pred_path in e.pred_paths:
            model_pred_saved_nets.append(load_seeded_data(model_pred_path, e, False))
...
def compute_corr_map(e, area_flat, pred, truth):
    cor_KE = (
        area_flat * pred[e.wet_bool].flatten() * truth[e.wet_bool].flatten()
    ).sum() / np.sqrt(
        (area_flat * pred[e.wet_bool].flatten() ** 2).sum()
        * (area_flat * truth[e.wet_bool].flatten() ** 2).sum()
    )
    return cor_KE
...
def get_timeseries_KE(
    ...
    for i in range(num_seeds):
        KE_net, KE_true = compute_KE(
            N_plot, e.test_data, model_pred_net[i], e.area, e.wet_bool
        )
...
def get_timeseries_temperature(
    ...
    T_test = np.array(e.test_data[:][1][:, 2] * e.std_out[2] + e.mean_out[2])
...
if args.N_test == 2000:
    T_true, T_saved = get_timeseries_temperature(
        e, model_pred_net, model_pred_saved_nets, start=999, N_eval=1999, long=True
    )
else:
    T_true, T_saved = get_timeseries_temperature(
        e, model_pred_net, model_pred_saved_nets, start=1999, N_eval=2999, long=True
    )
...
nino_true, nino_saved, amo_true, amo_saved = plot_indices(
    e, model_pred_net, model_pred_saved_nets, long=True
)
...
if args.N_test == 2000:
    pdf = get_pdf(
        e, model_pred_net, model_pred_saved_nets, start=999, N_days=1000, long=True
    )
else:
    pdf = get_pdf(
        e, model_pred_net, model_pred_saved_nets, start=1999, N_days=1000, long=True
    )
...
print("Plotting pdf...")
plot_metrics_pdf(
    e.pred_names + [e.network], e.region + "_Long_", e.output_dir, pdf, e.JUPYTER_MODE
)
