...
def compact_selectors(variables):
    selectors = []
    for variable in variables:
        physical_name, separator, level = variable.rpartition("_")
        selectors.append(
            (physical_name, int(level))
...
def test_flat_om4_reader_rejects_read_only_output_without_panicking(flat_om4_store):
    path, _ = flat_om4_store
    reader = open_reader(path, ["first"])
    output = np.empty((1, 1, 3, 5), dtype=np.float32)
...
def test_flat_om4_reader_rejects_invalid_index(flat_om4_store, indexes, match):
    ...
    with pytest.raises(RuntimeError, match=match):
        reader.read_into(indexes, ["first"], np.empty((1, 1, 3, 5), np.float32))
...
def test_flat_om4_reader_rejects_variable_not_opened(flat_om4_store):
    ...
    with pytest.raises(RuntimeError, match="was not opened"):
        reader.read_into([0], ["second"], np.empty((1, 1, 3, 5), np.float32))
...
def test_compact_om4_reader_translates_channels_and_preserves_order(
    ...
    actual = np.empty((3, len(channels), 3, 5), dtype=np.float32)
    reader.read_into([3, 0, 2], compact_selectors(channels), actual)
    expected = np.stack(
        [
...
def test_compact_om4_reader_rejects_read_only_output_without_panicking(
    compact_om4_store,
):
    ...
    with pytest.raises(RuntimeError, match="could not be borrowed for writing"):
        reader.read_into([0], compact_selectors(["thetao_0"]), output)
...
def test_compact_om4_reader_accepts_time_before_level(tmp_path):
    ...
    actual = np.empty((1, len(channels), 2, 2), dtype=np.float32)
    reader.read_into([1], compact_selectors(channels), actual)
...
def test_compact_om4_reader_rejects_variable_not_opened(compact_om4_store):
    ...
    with pytest.raises(RuntimeError, match="variable selector.*was not opened"):
        reader.read_into(
            [0],
            compact_selectors(["thetao_1"]),
            np.empty((1, 1, 3, 5), np.float32),
        )
