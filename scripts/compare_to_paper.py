...
def main(pred_path: str, truth_path: str, label: str = "Ours") -> None:
    ...
    # Markdown table
    # ------------------------------------------------------------------
    print(f"\n# Comparison: {label} vs Samudra-2 (paper, 1°)\n", flush=True)
    print(f"| Metric | Paper (Samudra-2 1°) | {label} |", flush=True)
    print("|---|---|---|", flush=True)
    print(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pred", required=True, help="Path to predictions.zarr")
    parser.add_argument(
        "--truth",
        default="/scratch/am16581/data/om4_onedeg_v3/OM4.zarr",
    )
    parser.add_argument(
        "--label", default="Ours", help="Column label for the markdown table"
    )
    args = parser.parse_args()
    main(args.pred, args.truth, label=args.label)
