...
# ./.python-greene submitit_hydra.py $comp exp=eval_unet_global ckpt_path='/scratch/sd5313/M2Lines/emulator/Ocean_Emulator/.LOCAL/train/2024-04-11-test_train_convnextinvunet_global/saved_nets/unet_best_steps_4_global_21_Test_in_u_v_T_ext_tau_u_tau_v_t_ref__outu_v_T_N_train_4000_Lateral_Data_025_no_smooth.pt' network="unet_cnext_20" name="$(date +%F)-eval_unet_convnextinvunet_Global" unet.encoder.n_channels=[180,90,45] unet.decoder.n_channels=[45,90,180] unet.encoder.dilations=[1,2,4] unet.decoder.dilations=[4,2,1] +exp/unet/modules/blocks@model.encoder.conv_block=conv_next_block +exp/unet/modules/blocks@model.decoder.conv_block=conv_next_block
