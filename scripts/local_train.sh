...
# 1. Basic UNet Global
./.python-greene submitit_hydra.py $comp exp=train_unet_global testing=true name="$(date +%F)-test_train_basicunet_global_1" region=global_1 batch_size=16 scheduler=True unet.encoder.n_channels=[64,128,256,512] unet.decoder.n_channels=[512,256,128,64]

# 2. ConvNext UNet Global
# ./.python-greene submitit_hydra.py $comp exp=train_unet_global testing=true name="$(date +%F)-local_train_convnextinvunet_global_1" region=global_1 batch_size=4 scheduler=True unet.encoder.n_channels=[45,90,180] unet.decoder.n_channels=[180,90,45] unet.encoder.dilations=[1,2,4] unet.decoder.dilations=[4,2,1] exp/unet/modules/blocks@unet.encoder.conv_block=conv_next_block exp/unet/modules/blocks@unet.decoder.conv_block=conv_next_block

# 3. AdamUNet Global
# ./.python-greene submitit_hydra.py $comp exp=train_adamunet_global testing=true name="$(date +%F)-local_train_adamunet_global_1" region=global_1 batch_size=8 scheduler=True

# 4. Swin Transformer Global
# ./.python-greene submitit_hydra.py $comp exp=train_swin_global testing=true name="$(date +%F)-local_train_swin_global_1" region=global_1 batch_size=8 swin.embed_dim=48 scheduler=True
