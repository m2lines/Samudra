...
# # Swin Transformer
# ./.python-greene submitit_hydra.py $comp exp=train_swin testing=true name="$(date +%F)-test_train_swin" batch_size=32 swin.embed_dim=24 
...
# ./.python-greene submitit_hydra.py $comp exp=train_unet_global testing=true name="$(date +%F)-local_train_convnextinvunet_global" batch_size=8 scheduler=True unet.encoder.n_channels=[180,90,45] unet.decoder.n_channels=[45,90,180] unet.encoder.dilations=[1,2,4] unet.decoder.dilations=[4,2,1] exp/unet/modules/blocks@unet.encoder.conv_block=conv_next_block exp/unet/modules/blocks@unet.decoder.conv_block=conv_next_block 