...
class Eval:
    ...
    def compare_pred_lateral(self):
        def get_stats(
            ...
            try:
                ...
                print(
                    "Path does not exist. Make sure to set run_gen_pred to True in config."
                )
    ...
    def compare_short_pred_lateral(self):
        ...
        rmse_unet, corrs_unet, ACC_unet, KE_unet, _, _, _, _ = get_spred(
            self.unet_path,
            self.region,
            5,
            3,
    ...
    def plot_metrics(self):
        ...
        KE_spec_vit, KE_spec_true = gen_KE_spectrum(
            N_plot, self.test_data, model_pred_vit, self.grids, self.wet
        )
        KE_spec_unet, KE_spec_true = gen_KE_spectrum(
            N_plot, self.test_data, model_pred_unet, self.grids, self.wet
        )

        KE_vit, KE_true = compute_KE(
            N_plot, self.test_data, model_pred_vit, self.area, self.wet_bool
        )
        ...
        enst_spec_vit, enst_spec_true = gen_enstrophy_spectrum(
            N_plot, self.test_data, model_pred_vit, self.grids, self.wet, self.wet_lap
        )
        ...
        enst_vit, enst_true = gen_enstrophy(
            N_plot,
            self.test_data,
            model_pred_vit,
            self.dx,
        ...
        N_eval = 200
        corr_T_vit, corr_T_true = compute_corrs_single(
            N_eval,
            T_test,
            model_pred_vit[:, :, :, 2],
            self.area,
        ...
        RMSE_T_vit, RMSE_T_true = compute_RMSE_single(
            N_eval, T_test, model_pred_vit[:, :, :, 2], self.area, self.wet_bool
        )
        ...
        N_eval = 100
        ACC_T_vit, ACC_T_true = compute_ACC_single(
            N_eval,
            T_test,
            model_pred_vit[:, :, :, 2],
            self.clim[:, :, :, 2],
