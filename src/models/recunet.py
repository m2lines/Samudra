...
class TransposedConvUpsample(torch.nn.Module):
    def __init__(
        ...
        upsampler.append(
            torch.nn.ConvTranspose2d(in_channels, out_channels, kernel_size=upsampling, stride=upsampling, padding=0) # check padding
        )
...
class BasicConvBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        for n in range(n_layers):
            convblock.append(torch.nn.Conv2d(in_channels=in_channels if n==0 else latent_channels,
...
class ConvGRUBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_gates = torch.nn.Conv2d(in_channels=in_channels + self.channels,
                                          out_channels=2*self.channels,  # for update_gate,reset_gate respectively
                                          kernel_size=kernel_size,
                                          padding="same")
        self.conv_can = torch.nn.Conv2d(in_channels=in_channels+self.channels,
...
class ConvNeXtBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        if in_channels == out_channels:
            ...
            self.skip_module = torch.nn.Conv2d(in_channels=in_channels,
                                              out_channels=out_channels,
                                              kernel_size=1)
        # Convolution block
        convblock = []
        # 7x7 convolution increasing channels
        convblock.append(torch.nn.Conv2d(
...
class UNetEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        for n, curr_channel in enumerate(n_channels):
            ...
            modules.append(ConvNeXtBlock(in_channels=old_channels,
...
class RecUNet(torch.nn.Module):
    ...
    def _initialize_hidden(self, inputs: Sequence, outputs: Sequence, step: int) -> None:
        ...
        for prestep in range(self.presteps):
            if step < self.presteps:
                s = step + prestep
                input_tensor = self._reshape(inputs[:, s*self.input_time_dim:(s+1)*self.input_time_dim]) # Assuming B, T, C, H, W
                if self.verbose:
    ...
    def forward(self, inputs: Sequence, output_only_last=False) -> torch.Tensor:
        ...
        for step in range(self.integration_steps):
            ...
            # Residual prediction
            reshaped = self._reshape_output(input_tensor[:, :self.output_channels*self.input_time_dim] + decodings)
