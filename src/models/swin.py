...
class SwinTransformer(torch.nn.Module):
    def __init__(self,
        ...
        if img_size[0] % (patch_size**2) != 0:
            img_size[0] = (int(img_size[0] / (patch_size**2)) + 1) * (patch_size**2)

        if img_size[1] % (patch_size**2) != 0:
            img_size[1] = (int(img_size[1] / (patch_size**2)) + 1) * (patch_size**2)

        self.resize_fn = Resize(img_size)
        self.input_size = pretrain_img_size
        self.encoder = SwinTransformerBackbone(img_size, patch_size, in_channels, embed_dim, depths, num_heads, window_size,
                                                mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate, norm_layer,
                                                ape, patch_norm, out_indices, frozen_stages, False)
        
        n_channels = [embed_dim*(2**i) for i in range(len(depths)-1,-1,-1)]
        self.decoder = SwinTransformerDecoder(conv_block, up_sampling_block, output_layer, n_channels, n_layers, output_channels, dilations)
