...
from .modules.blocks import CoreBlock, BilinearUpsample, TransposedConvUpsample, ViTBlock
...
class UNetWithAttention(BaseModel):
    def __init__(
        self,
        core_block: DictConfig,
        down_sampling_block: DictConfig,
        ...
        layers = []
        for i, (a, b) in enumerate(pairwise(ch_width)):
            if i < len(ch_width) // 2:  # Use regular convolutional blocks
                layers.append(
                    instantiate(
                        core_block,
                        a,
            ...
            else:  # Replace deeper layers with ViT blocks
                if i == 2:
                    layers.append(ViTBlock(in_channels=a, embed_dim=b, patch_size=5, num_heads=5))
                else:
                    layers.append(ViTBlock(in_channels=a, embed_dim=b, patch_size=2, num_heads=2))

            layers.append(instantiate(down_sampling_block))
        
        # layers.append(ViTBlock(in_channels=b))  # Deepest layer with ViT
        layers.append(instantiate(up_sampling_block, in_channels=b, out_channels=b))
    ...
    def forward_once(self, fts):
        ...
        for l in self.layers:
            ...
            fts = l(fts)
            if count < self.num_steps:
                if isinstance(l, CoreBlock) or isinstance(l, ViTBlock):
                    temp[count] = fts
                    count += 1
            elif count >= self.num_steps:
                if isinstance(l, BilinearUpsample) or isinstance(
