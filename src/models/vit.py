...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels=9, patch_size=16, emb_size=768, img_size=(128, 144)):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, (img_size[0]*img_size[1]) // (patch_size**2) +1, emb_size))
        self.patch_to_embedding = nn.Linear(patch_size*patch_size*in_channels, emb_size)
...
class VisionTransformer(nn.Module):
    def __init__(self, in_channels=9, patch_size=16, emb_size=768, num_layers=6, num_heads=8, output_channels=3, img_size=[128, 144]):
        super().__init__()

        if not isinstance(img_size, list): # Possibility of passing parameters through omegaconf. Need to convert to native list. 
        ...
        self.to_pixel_values = nn.Sequential(
            ...
            Rearrange('b (h w) (p1 p2 c) -> b c (h p1) (w p2)', h=img_size[0]//patch_size, w=img_size[1]//patch_size, p1=patch_size, p2=patch_size, c=output_channels),
            Resize(img_size)
        )
