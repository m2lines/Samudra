...
class TensorMap(Multiton):
    ...
    def get_output_channel_metadata(self, hist: int) -> torch.Tensor:
        """
        Generate metadata for each output channel.

        Returns a tensor of shape (num_output_channels, 3) where each row contains:
        - depth in meters (or 0 for 2D variables like zos)
        - variable kind as an integer (0 for uo, 1 for vo, 2 for thetao, 3 for so, 4 for zos)
        - time delta in days (for hist=1: -5 or 0, for hist=2: -10, -5, or 0)

        Args:
            hist: Number of historical timesteps in the output
        """
        num_output_channels = (hist + 1) * len(self.prognostic_var_names)
        metadata = torch.zeros(num_output_channels, 3)

        # Map variable names to integer kinds
        var_kind_map = {"uo": 0, "vo": 1, "thetao": 2, "so": 3, "zos": 4}

        idx = 0
        for time_idx in range(hist + 1):
            # Time delta: goes from -hist*5 to 0 in steps of 5 days
            time_delta = (time_idx - hist) * TIME_DELTA

            for var_name in self.prognostic_var_names:
                var_split = var_name.split("_")
                base_var = var_split[0]

                # Get variable kind
                var_kind = var_kind_map.get(base_var, -1)

                # Get depth
                if len(var_split) == 1:
                    # 2D variable (like zos)
                    depth = 0.0
                else:
                    # 3D variable with depth index
                    depth_idx = int(var_split[-1])
                    depth = DEPTH_LEVELS[depth_idx]

                metadata[idx, 0] = depth
                metadata[idx, 1] = var_kind
                metadata[idx, 2] = time_delta
                idx += 1
