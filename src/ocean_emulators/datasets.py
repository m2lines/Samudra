...
class _NormCache:
    # Normalization stats are cached as flat vectors for use in OceanData._normalize().
    # We keep both a CPU and (optionally) device copy to avoid repeated xarray -> numpy
    # conversions and repeated host->device transfers during training.
...
class TorchTrainDataset(Dataset[RawTrainData]):
    ...
    def __init__(
        ...
        # This avoids repeatedly calling OceanData.from_data_source(), which rebuilds
        # means/std tensors from xarray on every batch/step (high CPU RSS + overhead).
        self._cpu_cache = _NormCache(
            prognostic_means=[_flatten(s.means).float() for s in self.prognostic_srcs],
            prognostic_stds=[_flatten(s.stds).float() for s in self.prognostic_srcs],
            boundary_means=_flatten(self.boundary_src.means).float(),
            boundary_stds=_flatten(self.boundary_src.stds).float(),
            wet_prognostic=self.wet_prognostic,
    ...
    def __getitem__(self, idx: int):
        ...
        for step in range(self.steps):
            ...
            input_selected = self.prognostic_srcs[0].data.isel(time=past_index)
            label_selected = self.prognostic_srcs[-1].data.isel(time=future_index)
            boundary_selected = self.boundary_src.data.isel(time=past_index)
