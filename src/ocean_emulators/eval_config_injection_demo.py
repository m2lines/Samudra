...
from ocean_emulators.config import EvalConfig, ExperimentConfig, SamudraConfig
...
@cfg.inject("samudra")
@cfg.inject("data.hist", as_key="hist")
def make_model(
    num_in,
    num_out,
    wet,
    ...
    # Model
    logger.info(f"Getting model {experiment.network}")
    if "Samudra" == experiment.network:
        if samudra.ch_width[0] != num_in:
            logger.info(
                f"NOTE: Changing input channels to match data "
                f"{samudra.ch_width[0]}->{num_in}"
            )
            samudra.ch_width[0] = num_in
        if samudra.n_out != num_out:
            logger.info(
                f"NOTE: Changing output channels to match data "
                f"{samudra.n_out}->{num_out}"
            )
            samudra.n_out = num_out
        model = Samudra(
            samudra,
            hist=hist,
            wet=wet.to(device),
