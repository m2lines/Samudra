...
class FourierFeatures2D(nn.Module):
    ...
    def __init__(self, num_freq_bands: int, max_freq: float | None = None) -> None:
        super().__init__()
    ...
    def features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        max_freq = self.max_freq if self.max_freq is not None else max(ph, pw)
        ...
        leading = x.shape[:-3]
        return enc.expand(*leading, *enc.shape)
