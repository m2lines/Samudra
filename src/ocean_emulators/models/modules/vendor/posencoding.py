...
from ocean_emulators.models.modules.vendor.fourier import FourierExpansion

__authors__ = ["@wesselb", "@isaaccorley"]
...
def patch_root_area(
    ...
    Args:
        lat_min (torch.Tensor): Minimum latitudes of patches.
    ...
    #
    assert (lat_max > lat_min).all(), (
        f"lat_max - lat_min: {torch.min(lat_max - lat_min)}."
    )
    assert (lon_max > lon_min).all(), (
        f"lon_max - lon_min: {torch.min(lon_max - lon_min)}."
    )
...
def pos_scale_enc_grid(
    ...
    pos_encode = torch.cat((encode_h, encode_w), dim=-1)  # (B, L, D)
...
def lat_lon_meshgrid(lat: torch.Tensor, lon: torch.Tensor) -> torch.Tensor:
    ...
    stacked_grid = torch.stack(grid, dim=0)
    permuted_grid = stacked_grid.permute(0, 2, 1)
...
def pos_scale_enc(
    ...
    if isinstance(patch_dims, list):
        patch_dims = tuple(patch_dims)
    assert isinstance(patch_dims, tuple) and len(patch_dims) == 2, (
        "`patch_dims` must be a tuple of two integers."
    )
