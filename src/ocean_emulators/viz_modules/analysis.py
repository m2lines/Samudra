...
def compute_ohc(temperature: xr.DataArray, areacello: xr.DataArray, 
                dz: xr.DataArray, c_p: float = 3850, rho_0: float = 1025, 
                zeta_joules_factor: float = 1e21) -> xr.DataArray:
    """
    ...
    """
    ohc = (temperature * c_p * rho_0 / zeta_joules_factor).weighted(
        areacello * dz
    ).sum(["lev"])
...
def nino_index_compute_clim(T: xr.DataArray, area: xr.DataArray, dt: int = 5, 
                           window: int = 150) -> Tuple[xr.DataArray, xr.DataArray]:
    """
...
def compute_seasonal_cycle(ds: xr.Dataset, groupby_coord: str = "time.month") -> xr.Dataset:
    """
...
def compute_profile_metrics(pred_profiles: Dict[str, xr.Dataset], 
                           truth_profile: xr.Dataset) -> Dict[str, Dict[str, xr.Dataset]]:
    """
...
def compute_basin_statistics(ds: xr.Dataset, basin_masks: xr.Dataset) -> Dict[str, xr.Dataset]:
    """
...
def compute_depth_integrated_quantities(ds: xr.Dataset, variable: str, 
                                       max_depth: float = None) -> xr.DataArray:
    """
