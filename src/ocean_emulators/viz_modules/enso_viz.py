...
def nino34_index_xr(
    sst_data: xr.DataArray, area_weights: xr.DataArray, dt: int = 5, window: int = 150
...
def plot_nino34_index_over_time_v2(
    nino_gt: xr.DataArray,
    nino_preds: dict[str, xr.DataArray],
    titles: list[str],
    colors: list[str],
    output_path: str,
    ...
    for i, (key, nino_pred) in enumerate(nino_preds.items()):
        ...
        ax1.plot(
            pred_time_vals, nino_pred.values, color=color, linewidth=2, label=title
        )

    ax1.axhline(y=0, color="gray", linestyle="--", alpha=0.7)
    ax1.axhline(
        y=0.5, color="red", linestyle="--", alpha=0.5, label="El Niño threshold"
    )
    ax1.axhline(
        y=-0.5, color="blue", linestyle="--", alpha=0.5, label="La Niña threshold"
    )
    ...
    ax2.fill(
        nino34_lon, nino34_lat, color="red", alpha=0.2, transform=ccrs.PlateCarree()
    )
...
def generate_enso_visualizations(
