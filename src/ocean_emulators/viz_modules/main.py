...
class OceanVisualizationPipeline:
    ...
    def __init__(self, config: Union[Dict[str, Any], VizConfig]):
        """
    ...
    def _get_cache_path(self, dataset_type: str, key: str = None) -> str:
        ...
        if dataset_type == "groundtruth":
            ...
            path_hash = hashlib.md5(self.config.groundtruth_path.encode()).hexdigest()[:8]
    ...
    def _load_cached_profiles(self) -> bool:
        ...
        try:
            ...
            for k in self.pred_dict_processed.keys():
                ...
                print(f"Loaded cached prediction profile for {k} from {pred_cache_path}")
    ...
    def show_cache_status(self) -> None:
        ...
        for cache_file in cache_files:
            try:
                size = sum(f.stat().st_size for f in cache_file.rglob('*') if f.is_file())
                total_size += size
    ...
    def _save_climate_indices(self, nino_gt: xr.DataArray, 
                             nino_preds: Dict[str, xr.DataArray]) -> None:
    ...
    def _save_metrics_summary(self, profile_metrics: Dict[str, Dict[str, xr.Dataset]], 
    ...
    def _create_original_metrics_files(self, profile_metrics: Dict[str, Dict[str, xr.Dataset]], 
                                      metrics_path: str) -> None:
        """Create the specific metrics files that match original viz.py naming."""
            ...
            for pred_key, metrics in profile_metrics.items():
                if "mae" in metrics and "thetao" in metrics["mae"].data_vars:
                    ...
                    if "lev" in metrics["mae"]["thetao"].dims:
                        sst_mae_value = metrics["mae"]["thetao"].isel(lev=0).mean().compute().item()
                    else:
        ...
        salinity_deseasonalized_file = os.path.join(metrics_path, "salinity_deseasonalized_info.txt")
            ...
            for pred_key in profile_metrics.keys():
                if pred_key in self.pred_dict_processed:
                    ...
                    if "so" in pred_data.data_vars:
                        ...
                        salinity_pred = (
                            pred_data["so"]
                            .weighted(self.ds_groundtruth["areacello"] * self.ds_groundtruth["dz"])
                            .mean(["x", "y", "lev"])
                        )
                        ...
                        coeffs_salinity_pred_trend = np.polyfit(
                            np.arange(salinity_deseasonalized.size), salinity_deseasonalized, 1
                        )
                        pred_name = self.pred_dict_processed[pred_key].get("name", pred_key)
    ...
    def run_config_based_analysis(self) -> None:
        ...
        print(f"Starting Ocean Visualization Analysis with groups: {self.config.analysis_groups}")
        
        # Load and process data
        self.load_data()
        
        # Compute profiles if needed
        if any(group in self.config.analysis_groups for group in ["timeseries", "metrics"]):
            self.compute_profiles()
...
def run_visualization_pipeline(config: Union[Dict[str, Any], VizConfig], minimal: bool = False) -> str:
    """
