...
def compute_pdf(data: xr.DataArray, bins: int = 150, data_range: tuple = None) -> tuple:
    """
    ...
    try:
        ...
        for i, v in enumerate(variables):
            ...
            try:
                ...
                gt_sample = ds_groundtruth[v].isel(
                    time=slice(0, min(10, len(ds_groundtruth.time)))
                )
                ...
                for k, pred_data in pred_dict.items():
                    if v in pred_data["ds_prediction"].data_vars:
                        pred_sample = pred_data["ds_prediction"][v].isel(
                            time=slice(0, min(10, len(pred_data["ds_prediction"].time)))
                        )
                ...
                true_pdf, bins_true = compute_pdf(
                    gt_sample, bins=100, data_range=data_range
                )
                ...
                for j, (k, pred_data) in enumerate(pred_dict.items()):
                    if v in pred_data["ds_prediction"].data_vars:
                        ...
                        pdf_net, bins_net = compute_pdf(
                            pred_sample, bins=100, data_range=data_range
                        )

                        # Get prediction name
                        pred_name = (
                            titles[j] if j < len(titles) else pred_data.get("name", k)
                        )
        ...
        # Save the figure
        plt.savefig(
            os.path.join(pdfs_path, "PDF_Plots_Short.png"), bbox_inches="tight", dpi=600
        )
        plt.close()
        ...
        try:
            ...
            plt.savefig(
                os.path.join(pdfs_path, "PDF_Plots_Short.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
