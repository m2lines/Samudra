...
def create_timeseries_directories(
    output_path: str, variables: list[str] | None = None
) -> dict[str, str]:
    """
...
def plot_all_variable_timeseries(
    profile_groundtruth: xr.Dataset,
    pred_profiles: dict[str, xr.Dataset],
...
def plot_multi_variable_grid(
    ...
    colors: list[str],
    titles: list[str],
    plot_title: str = "Multi-variable Time Series",
) -> None:
    """
        ...
        for j, level in enumerate(depth_levels):
            ...
            if variable in profile_groundtruth.data_vars:
                ...
                if "lev" in profile_groundtruth[variable].dims:
                    ...
                    truth_data = profile_groundtruth[variable]
                # Convert cftime to pandas datetime for matplotlib compatibility
                truth_time_vals = pd.to_datetime(
                    [str(t) for t in truth_data.time.values]
                )
                ax.plot(
                    truth_time_vals, truth_data, "k-", linewidth=2, label="Ground Truth"
                )
                ...
                for k, (key, pred_profile) in enumerate(pred_profiles.items()):
                    if variable in pred_profile.data_vars:
                        ...
                        # Convert cftime to pandas datetime for matplotlib compatibility
                        pred_time_vals = pd.to_datetime(
                            [str(t) for t in pred_data.time.values]
                        )
                        ax.plot(
                            pred_time_vals,
                            pred_data,
...
def plot_temperature_profile_comparison(
...
def create_timeseries_summary_plot(
    profile_groundtruth: xr.Dataset,
    pred_profiles: dict[str, xr.Dataset],
    output_path: str,
    var_labels: dict[str, str],
...
def create_global_timeseries_plots(
...
def plot_temperature_timeseries_grid_shallow_both(
    profile_groundtruth: xr.Dataset,
    pred_profiles: dict[str, xr.Dataset],
    output_path: str,
    colors: list[str],
    ...
    if variable in profile_groundtruth.data_vars:
        for lev in shallow_levels:
            ...
            if "lev" in profile_groundtruth[variable].dims:
                closest_lev_idx = np.argmin(
                    np.abs(profile_groundtruth.lev.values - lev)
                )
                ...
                ax.plot(
                    truth_time_vals, truth_data, "k-", linewidth=2, label=dataset_name
                )
                ...
                for i, (key, pred_profile) in enumerate(pred_profiles.items()):
                    if variable in pred_profile.data_vars:
                        pred_closest_lev_idx = np.argmin(
                            np.abs(pred_profile.lev.values - lev)
                        )
                        pred_data = pred_profile[variable].isel(
                            lev=pred_closest_lev_idx
                        )
                        color = colors[i % len(colors)]
                        title = titles[i] if i < len(titles) else key
                        pred_time_vals = pd.to_datetime(
...
def plot_temp_timeseries_grid_shallow_skipped(
    profile_groundtruth: xr.Dataset,
    pred_profiles: dict[str, xr.Dataset],
    output_path: str,
    colors: list[str],
...
def generate_all_timeseries_plots(
    profile_groundtruth: xr.Dataset,
    pred_profiles: dict[str, xr.Dataset],
    output_path: str,
    var_labels: dict[str, str],
    ...
    titles: list[str],
    variables: list[str] | None = None,
    ds_groundtruth: xr.Dataset | None = None,
    pred_dict_processed: dict[str, dict] | None = None,
) -> None:
    """
    ...
    summary_path = os.path.join(
        directories["timeseries"], "timeseries_grid_shallow_all_vars.png"
    )
    ...
    profile_path = os.path.join(
        directories["timeseries"], "temperature_profiles_comparison.png"
    )
    plot_temperature_profile_comparison(datasets, dataset_titles, profile_path)
