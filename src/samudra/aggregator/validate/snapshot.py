...
from samudra.aggregator.plotting import plot_paneled_data
...
class SnapshotAggregator(ValidateSubAggregator):
    ...
    def __init__(
        self, metadata: dict[str, dict[str, str]] | None = None, input_steps: int = 1
    ):
        """
        Args:
            metadata: Mapping of variable names their metadata that will
                used in generating logged image captions.
            input_steps: Number of raw input timesteps.
        """
        if metadata is None:
            metadata = {}
        else:
