...
from samudra.datasets import BatchReadUse, TrainBatchPreparer, TrainData, TrainingShard
...
class _NativeOm4CanonicalReader:
    ...
    below; physical layout and time indices never become CanonicalDataset state.
    """
    ...
    def statistics(self) -> ChannelStatistics:
        return self.semantic.statistics
    ...
    def select_channels(self, channels: tuple[str, ...]) -> Self:
        ...
        return replace(
            self,
            semantic=self.semantic.select_channels(channels),
            channels=channels,
        )
    ...
    def read(self, request: CanonicalReadRequest) -> LoadedPlanes:
        # Keep the ordinary CanonicalDataset path independent from the optimized
    ...
    def read_static(self, names):
        return self.semantic.read_static(names)

    def metadata(self, dataset_spec):
        return self.semantic.metadata(dataset_spec)
    ...
    def read_unique(
        ...
        *,
        ...
        reader_variables = [
            self._reader_variables[name].extension_selector() for name in self.channels
        ]
        shape = (len(physical_time_indices), len(self.channels), *self._spatial_shape)
        tensor = (
...
def native_om4_dataset(
    ...
    for logical_name in dataset.channels:
        ...
        for physical_name in physical.data_vars:
            ...
            if dataset.dataset_spec.depth_levels.index(depth) == int(level_text):
                matches.append(name)
        if len(matches) == 1:
    ...
    return replace(dataset, _reader=reader)


def _native_reader(source: CanonicalDataset) -> _NativeOm4CanonicalReader:
...
class _RustBatchDataset:
    ...
    def __init__(
        ...
        self._input_reader = _native_reader(self.shard.prognostic_srcs[0])
        self._boundary_reader = _native_reader(self.shard.boundary_src)
        self._label_reader = _native_reader(self.shard.prognostic_srcs[-1])
    ...
    def load_chunk_batch(
        ...
        *,
        ...
        def pending_use(
            ...
            key = (reader.storage_id, reader.channels)
...
class RustTrainDataLoader:
    ...
    def _prepare_batch(
        ...
        train_data = preparer.new_train_data(self._device)
