...
def train_one_epoch(epoch, model, train_loader, N_in, N_extra, hist, loss_fn, optimizer, scheduler, steps, weight, device, wandb_flag):
    model.train(True)
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(train_loader, 1, header)):
