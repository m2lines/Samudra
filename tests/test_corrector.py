...
def corrector_init():
    ...
    data = xr.Dataset(
        {
            "var_0": (["lat", "lon"], np.random.randn(2, 2)),
            "var_1": (["lat", "lon"], np.random.randn(2, 2)),
            "var_2": (["lat", "lon"], np.random.randn(2, 2)),
        },
    ...
    masks = Masks.from_floating(prognostic=wet_mask, boundary=wet_mask)
