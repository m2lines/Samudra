...
class TestStochasticDepthManager:
    ...
    def test_config_validation_negative_multipliers(self):
        """Test that negative stage multipliers are rejected."""
        with pytest.raises(ValueError, match="Stage multipliers must be non-negative"):
            StochasticDepthConfig(
                drop_path_rate=0.1,
                early_dropout_epochs=10,
                per_stage_multipliers=[0.5, -1.0, 2.0],  # Negative multiplier
            )

    def test_config_validation_valid_multipliers(self):
        """Test that valid stage multipliers are accepted."""
        config = StochasticDepthConfig(
            drop_path_rate=0.1,
            early_dropout_epochs=10,
            per_stage_multipliers=[0.0, 1.0, 2.0],  # Zero is valid
        )
