...
def compact_om4_source(tmp_path):
    ...
    layout = dataclasses.replace(
        build_om4_layout(prognostic_vars_key="thetao_1", boundary_vars_key="hfds"),
        prognostic_var_names=prognostic_variables,
    )
    time_size = 20
    level_size = len(layout.depth_levels)
    ...
    coords = {
        ...
        "lev": np.asarray(layout.depth_levels),
    ...
    canonicalizer = Om4DataSourceConfig(
        train_time=Om4TimeConfig.model_validate(
            {"start": "2000-01-01", "end": "2000-02-01"}
        ),
        val_time=Om4TimeConfig.model_validate(
            {"start": "2000-02-01", "end": "2000-03-01"}
        ),
        data_location=UnresolvedLocation(path=str(data_path)),
        data_means_location=UnresolvedLocation(path=str(means_path)),
        data_stds_location=UnresolvedLocation(path=str(stds_path)),
